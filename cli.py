def run() :
    ...
    import src.util
    import src.cli.chat
    import src.cli.extract
    import src.cli.init_plugin
    import src.cli.mobile_plugin
    import src.cli.os_plugin
    import src.cli.auto_db
    ...
    while True :
        ...
        if num == 1 :
            ...
            src.cli.init_plugin.iphone_information(backup_file_manifest, backup_file_info)
        elif num == 2 :
            log("[Selected iPhone Backup Information]")
            src.cli.init_plugin.iphone_backup_information(backup_file_manifest, backup_file_info)
        elif num == 3 :
            log("[Selected iPhone Accessibility Information]")
            src.cli.init_plugin.iphone_accessibility_information(backup_file_manifest, backup_file_info)
        elif num == 4 :
            log("[Selected Installed Application]")
            src.cli.init_plugin.installed_Application(backup_file_manifest, backup_file_info)
        elif num == 5 :
            log("[Selected Extract Backup File]")
            src.cli.extract.extract_backupfile(backup_file_location)
        elif num == 9 :
            ...
            if artifacts == 0 :
                ...
                src.cli.mobile_plugin.owner_infomation_artifact()
            elif artifacts == 1 :
                log("[Selected AddressBook Artifact]")
                src.cli.mobile_plugin.addressbook_artifact()
            elif artifacts == 2 :
                log("[Selected Calendar Event Artifact]")
                src.cli.mobile_plugin.calendar_event_artifact()
            elif artifacts == 3 :
                log("[Selected Installed Application Artifact]")
                src.cli.mobile_plugin.installed_application()
            elif artifacts == 4 :
                log("[Selected Apple Accounts Artifact]")
                src.cli.os_plugin.apple_accounts()
            elif artifacts == 5 :
                log("[Selected Sim Card Artifact]")
                src.cli.os_plugin.sim_card()
            elif artifacts == 6 :
                log("[Selected Bluetooth Artifact]")
                src.cli.os_plugin.bluetooth()
            elif artifacts == 6.1 :
                log("[Selected Bluetooth That Have Been Shown Artifact]")
                src.cli.os_plugin.bluetooth_that_have_been_shown()
            elif artifacts == 7 :
                log("[Selected SMS Artifact]")
                src.cli.chat.sms()
            elif artifacts == 8 :
                log("[Selected App Permission]")
                src.cli.os_plugin.app_permission()
            elif artifacts == 9 :
                log("[Selected Wallet Pass]")
                src.cli.os_plugin.wallet_pass()
            else :
                print("\nError, Wrong Number. Please Check Your Number.\n")
                log("Error, Wrong Number. Please Check Your Number.")
                break
        elif num == 99 :
            src.cli.auto_db.auto(backup_file_manifest,backup_file_info)
        elif num == 0 :
            print("Bye!")
            log("Bye!")
            break
